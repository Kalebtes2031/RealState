  ...
  return (
    <View className="flex-1">
      <ScrollView className="flex-1  pb-12 bg-white">
        ...
        <View className="mt-5 mx-5 bg-gray-50  rounded-xl px-4 flex flex-row items-center justify-between">
          <View className="flex flex-row items-center justify-center gap-x-4">
            <Ionicons name="search" size={20} color="gray" />
            <TextInput placeholder="Beijing China" />
          </View>
        ...
        <ScrollView
          ...
          className="mt-6 pl-5"
        >
          <View className="mr-4 rounded-full bg-secondary px-6 py-3">
            <Text className="text-white font-semibold">All</Text>
          </View>
          {texts.map((text, idx) => (
            <View
              key={idx}
              className="mr-4  rounded-full bg-blue-100 px-6 py-3"
            >
              <Text className="text -gray-600 font-semibold">{text.text}</Text>
            </View>
          ))}
        </ScrollView>
      ...
      <BottomModal visible={isModalVisible} onClose={closeBottomModal}>
        ...
        <View className="flex flex-col justify-center items-center">
          <View className="flex flex-row justify-between w-[289px]">
            <Text className="font-bold text-[16px] text-secondary">${values[0]}</Text>
            <Text className="font-bold text-[16px] text-secondary">${values[1]}</Text>
          </View>
          <MultiSlider
            values={values}
            sliderLength={280}
